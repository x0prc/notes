...
- `continueSession` - By default sessions expire after the command execution.
- `decrypt`, `encrypt` : TPM will encrypt the first parameter of its response to the command.
### wolfTPM Flow to enable MiTM protection
- Create Primary Key
- Create TPM session of HMAC type
- Set session attributes (TPMA_Session)
	- `&dev` . `&tpmsession` .`mySessionAttributes`
- Execute a TPM command
- Flush TPM session and objects.
- ![[Pasted image 20251211224356.png]]
- Salted Session : Session key is derived using a loaded asymmetric key in the TPM.
- Bound Session : Session key is derived from authorization value of bind entity.